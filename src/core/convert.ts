    ...
    rows.flatMap((row) => {
        if (isRealCommand(row)) {
            if (Array.isArray(row.args)) {
                return [{ key: row.key, args: filterRealCommandsRecursively(row.args) }]
            } else {
                return [row]
            }
        } else {
...
const applyMacroToCommandNodes = (rows: Array<RuntimeCommandLike>, macro: Macros): Array<RuntimeCommandLike> =>
    rows.flatMap((row) => {
        if (Array.isArray(row.args)) {
            return [{ key: row.key, args: applyMacroToCommandNodes(row.args, macro) }]
...
const applyMacrosToCommandsNodes = (rows: Array<RuntimeCommandLike>): Array<RuntimeCommandLike> =>
    rows.flatMap((row) => {
        if (Array.isArray(row.args)) return [{ key: row.key, args: applyMacrosToCommandsNodes(row.args) }]
