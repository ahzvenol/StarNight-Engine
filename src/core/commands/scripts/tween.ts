...
const tween: CommandRunFunction = ({ timer }) => ({ target, duration, transition, ...args }) => {
    const ease = ((EASE_KEYS as unknown as Array<string>).includes(transition) ? transition : "linear") as typeof EASE_KEYS[number]
    if (!(target in activeTweens)) activeTweens.set(target, createjs.Tween.get(target))
    const tween = activeTweens.get(target)!
    tween.to(omit(args, ['@']), duration, (createjs.Ease as { [key in typeof EASE_KEYS[number]]: Function })[ease])
