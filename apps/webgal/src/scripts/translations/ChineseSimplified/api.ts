 ...
 * @remarks
 * - `缓动时间` 仅控制背景图片的入场动画时长（淡入效果），其他属性（如位置、缩放、滤镜等）会立即应用。
 * - 参数值支持数值或字符串（如 "+=100"），+= 表示相对于当前值的增量，但并不是所有参数都可以使用 +=
 * @param 参数对象
 * @param .资源路径 - 背景图片的文件路径（必需）
 * @param .缓动时间 - 入场动画持续时间，单位毫秒（可选，默认 225）
 ...
 * @example
 * 设置背景({ 资源路径: "/咸鱼池塘.jpg", X坐标: -120, 缓动时间: 500, 模糊: 5, 亮度: 1.2 })
 */
 ...
 * - `缓动时间` 仅控制立绘图片的入场动画时长（淡入效果），其他属性（如位置、缩放、滤镜等）会立即应用。
 * - 参数值支持数值或字符串（如 "+=100"），+= 表示相对于当前值的增量，但并不是所有参数都可以使用 +=
 * @param 参数对象
 ...
 * @param .变换矩阵 - 自定义变换矩阵，类型为 PixiPlugin.PixiMatrix（可选）
 * @example
 * 设置立绘({ 标识符: "咸鱼", 资源路径: "/saltfish/fish.jpg", X坐标: 640, Y坐标: 640, 缓动时间: 500, 模糊: 5 })
 */
 ...
 * @param .缓动时间 - 动画持续时间，单位毫秒（可选，默认 0，无动画）
 ...
 * @param .变换矩阵 - 自定义变换矩阵，类型为 PixiPlugin.PixiMatrix（可选）
 * @example
 * 添加动画({ 作用目标: "咸鱼", X坐标: "+=100", Y坐标: 720, 缓动时间: 1000, 缓动函数: "M0,0,C0,0,1,1,1,1" })
 */
 ...
 * @param .缓动时间 - 淡出动画的持续时间，单位毫秒（可选，默认 225）。
 * @example
 * // 关闭单个立绘
 * 关闭图像({ 作用目标: "咸鱼" })
 * // 同时关闭多个立绘
 ...
 * @example
 * 设置配乐({ 资源路径: "/bgm01.mp3", 缓动时间: 1000, 音量: 0.5 })
 */
 ...
 * @param .使用HTML5 - 是否使用 HTML5 音频播放，适合大文件以减少加载时间（可选，默认自动选择）
 * @example
 * 设置音效({ 资源路径: "/se01.mp3", 循环播放: true, 缓动时间: 500, 音量: 0.7 })
 */
 ...
 * @remarks
 * - `缓动时间` 控制音量渐变的过渡时间，未设置时音量立即应用。
 * @param 参数对象
 * @param .作用目标 - 音轨的标识符（必需）
 * @param .音量 - 目标音量，范围 0 到 1（必需）
 * @param .缓动时间 - 音量渐变过渡时间，单位毫秒（可选）
 * @example
 * 设置音量({ 作用目标: "bgm", 音量: 0.5, 缓动时间: 1000 })
 */
 ...
 * @remarks
 * - `缓动时间` 控制音量渐出的过渡时间，未设置时立即关闭。
 * @param 参数对象
 * @param .作用目标 - 要关闭的音轨标识符（必需）
 * @param .缓动时间 - 音量渐出过渡时间，单位毫秒（可选）
 * @example
 * 关闭音频({ 作用目标: "bgm", 缓动时间: 1000 })
 */
 ...
 * @param .缓动时间 - 动画持续时间，单位毫秒（可选，默认 0）
 * @param .缓动函数 - 动画缓动函数，支持 GSAP 缓动预设、缓动曲线或函数（可选，默认 "none"）
...
export const 基本动画 = Api(
    Alias(
        ...
        { id: '标识符', target: '作用目标', duration: '缓动时间', mode: '模式', ease: '缓动函数', position: '位置' } as const
    )
)
