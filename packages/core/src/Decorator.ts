...
export function Dynamic<T = void, R = void>(fn: DynamicCommand<T, R>): StandardCommand<T, R> {
    return (args) => async (context) => {
...
// 默认阻塞的动态命令
export function DynamicBlocking<T = void, R = void>(fn: DynamicCommand<T, R>): StandardCommand<T, R> {
    return (args) => async (context) => {
        const { onActRush: rush, onGameStop: stop } = context
        const generator = fn(context)(args)
        return normalize(() => run(generator, { rush, stop })) as Promise<R>
...
export function NonBlocking<T = void, R = void>(fn: NonBlockingCommand<T, R>): StandardCommand<T, R> {
    return (args) => async (context) => normalize(() => fn(context)(args))
}

// 需要等待用户输入的命令,在完成命令行为之前不能解除阻塞
export function Blocking<T = void, R = void>(fn: BlockingCommand<T, R>): StandardCommand<T, R> {
    return (args) => async (context) => normalize(() => fn(context)(args))
}
...
export function Macro<T = void, R = void>(fn: MacroCommand<T, R>): StandardCommand<T, R> {
